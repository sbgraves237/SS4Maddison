...
```{r findData}
# I think the following should work but doesn't:
# (SS4xlsx <- fs::path_package('SS4Maddison', 
#              'extdata', 'mpd2023_web.xlsx'))

dataName <- '^mpd2023.*xlsx$'
(SS4vig_wd <- getwd())
(SS4xlsx <- dir(SS4vig_wd, dataName, 
                  full.names = TRUE))
```

```{r findD2}
if(length(SS4xlsx)<1){
# Is working data vignette directory? If yes, try parent  
  (SS4vig_split <- strsplit(SS4vig_wd, '/')[[1]])
  if(tail(SS4vig_split, 1) == 'vignettes'){
    SS4vig_wd <- paste(head(SS4vig_split, -1), 
                collapse='/') 
  }
  (SS4xlsx <- dir(SS4vig_wd, dataName, 
                        full.names = TRUE))
  if(length(SS4xlsx)<1){
    (SS4extdata <- paste0(SS4vig_wd, '/extdata'))
    (SS4xlsx <- dir(SS4extdata, dataName, 
                        full.names = TRUE))
    if(length(SS4xlsx)<1){
      (SS4inst <- paste0(SS4vig_wd, '/inst'))
      (SS4xlsx <- dir(SS4inst, dataName, 
                        full.names = TRUE))
      if(length(SS4xlsx)<1){
        (SS4extdata <- paste0(SS4inst, '/extdata'))
        (SS4xlsx <- dir(SS4extdata, dataName, 
                        full.names = TRUE))
      }
    }        
