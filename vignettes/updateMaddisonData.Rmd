...
How do `cd_Ctry` relate to the [`countrycode`](https://search.r-project.org/CRAN/refmans/countrycode/html/countrycode.html) function? 

```{r countrycodeFn}
if(length(SS4xlsx)>0){
  cdsFrFn <- countrycode::countrycode(
    cd_Ctry$country, 'country.name.en', 'iso3c')
# Warning: some ambig, e.g., Czech & Yugoslavia
  ctryFrFn <- countrycode::countrycode(
    cd_Ctry$countrycode, 'iso3c', 
      'country.name.en')
# Warning: Some ambig: CSK, SUN, YUG          
  ctryFrFn <- countrycode::countrycode(
    cd_Ctry$countrycode, 'iso3c', 'ecb')
# Warning: Some ambig: CSK, SUN, YUG          
}
```

Conclusion: Further experimentation might identify a way we can use the `countrycode` function with the Maddison data, but that does not seem likely at the moment. 
...
if(length(SS4xlsx)>0){
  ...
  MaddisonCountries <- cbind(cd_Ctry, 
            substring(ctryRgn, 4))
  saveRDS(MaddisonCountries, 'MaddisonCountries.rda')  
  dir()
}
```
...
nRowMadSrc <- nrow(MaddisonSources0)
MaddisonSources0[c(2, nRowMadSrc-(4:3)), ]
```
```